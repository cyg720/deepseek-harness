-- 授权中心 0.4.2 新库建表。
-- 来源：qishu/PRD/2-授权中心/授权中心.schema.sql（文档草案）。
-- 实施调整（相对草案）：
--   1. 账号与组织的更新时间触发器使用 BEFORE UPDATE OF <业务列>：登录只更新 last_login_at、
--      子树移动只重写 org_path 时不会递增 version，避免系统维护列使其他管理员的编辑版本失效。
--   2. 去掉草案中错位的 is_super_admin 列注释，归位到所属表。
-- 其余表、列、约束、索引、函数与草案逐字一致。
-- 表前缀 qs__auth__；时间列存北京时间秒精度；密码与令牌只存散列/摘要。
...
-- 账号：只在业务列变更时递增版本，登录只写 last_login_at 不产生版本变更。
CREATE TRIGGER qs__auth__account_update BEFORE UPDATE OF
    username, password, phone, email, logo, remark, account_type, status, ip_whitelist,
    created_by, updated_by, must_change_password
    ON qs__auth__account FOR EACH ROW EXECUTE FUNCTION qs__auth__maintain_update();

-- 组织：物化路径 org_path 由服务维护，子树移动时批量重写路径不应改动子节点的编辑版本。
CREATE TRIGGER qs__auth__organization_update BEFORE UPDATE OF
    parent_id, org_name, org_type, leader, sort_order, remark, created_by, updated_by
    ON qs__auth__organization FOR EACH ROW EXECUTE FUNCTION qs__auth__maintain_update();
