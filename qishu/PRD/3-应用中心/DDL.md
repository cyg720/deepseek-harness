...
1、应用中心（基础能力）
      应用管理：
            字段：应用名称、应用编码、logo、封面、应用作用域（个人/团队/企业  代表谁能看见该应用）、状态、版本、应用描述、dependencies（json数组）、app_key、app_secret、
            操作：新增、编辑、删除、详情、列表、发布审批、冻结应用、启用应用
            备注：【dependencies：如果应用依赖的插件、工具、skill等被卸载或升级，该应用可能崩溃；发布审批时，系统自动校验依赖是否存在，防止上线即报错】，app_key、app_secret 是鉴权中心注册后返回来的应用凭证，用于应用调用其他服务时的认证与授权
