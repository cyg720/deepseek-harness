...
check('版本0.4.0修订7', state['version'] == '0.4.0' and state['revision'] == 7)
check('30条用户反馈逐字保留', len(feedback) == 30 and all(row['feedback'] in body for row in feedback))
check('23个Q主项连续', re.findall(r'^\| (Q-\d{2}) \|', body.split('### 12.1')[1].split('### 12.2')[0], re.M) == [f'Q-{i:02}' for i in range(1, 24)])
...
check('30天令牌约束', "CHECK (expires_at = created_at + INTERVAL '30 days')" in sql)
...
indexes = re.findall(r'CREATE INDEX (\w+) ON (\w+)\((\w+)\)', sql)
...
check('30个现行AC均有执行记录', len(active_ac) == metadata['acceptancePoints'] == 30 and all('| ' + a + ' | 未执行；' in body for a in active_ac))
for old in [5, 6, 7, 8, 16, 20, 23]:
...
active_apis = set(re.findall(r'^\| (API-\d{2}) \|', body.split('### 5.2')[1].split('### 5.3')[0], re.M))
...
with tempfile.TemporaryDirectory(prefix='auth-v040-check-') as temp:
    ...
    check('固定参数可重复且与交付一致', data_bytes == (base / 'b/authorization-fixtures.json').read_bytes() == (ROOT / '测试数据/v0.4.0/authorization-fixtures.json').read_bytes())
