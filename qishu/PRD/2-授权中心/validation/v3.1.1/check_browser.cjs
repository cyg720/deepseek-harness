...
async function main() {
  ...
  async function newPage(file = htmlPath, viewport) {
    ...
    await p.waitForFunction(() => document.querySelectorAll('.diagram-view svg').length === 6 && !document.querySelector('[data-diagram-state="pending"]'), {timeout:30000});
    return p;
  }
  ...
  await test('原生 file:// 离线启动及完整正文', async () => {
    ...
    assert.ok(await p.locator('pre').count() >= 12);
    assert.equal(await p.locator('h4').filter({hasText:/TC-\d{2}-\d{3}/}).count(), 141);
  ...
  await test('六张 Mermaid 离线渲染为 SVG 且保留源码', async () => {
    const blocks = await p.locator('pre code').allTextContents();
    assert.equal(blocks.filter(x=>x.startsWith('flowchart TB')).length,1);
    assert.equal(blocks.filter(x=>x.startsWith('stateDiagram-v2')).length,5);
    const mdText=await md(p);
    for(const block of blocks) assert.ok(mdText.includes(block));
    await p.waitForFunction(() => document.querySelectorAll('.diagram-view svg').length === 6);
    assert.equal(await p.locator('.diagram-error').count(),0);
    await p.locator('#readBtn').click();
    const overview=p.locator('.section').filter({has:p.locator('h2').filter({hasText:'一图统揽'})});
    await overview.scrollIntoViewIfNeeded();
    await p.screenshot({path:path.join(out,'overview.png')});
  });
  await test('完整多级目录、父级折叠与DDL子标题定位',async()=>{
    const headings=await p.locator('.sectionbar h2,.sectionbody h1,.sectionbody h2,.sectionbody h3,.sectionbody h4,.sectionbody h5,.sectionbody h6').count();
    assert.equal(await p.locator('#toc a').count(),headings);
    const parent=p.locator('#toc a').filter({hasText:'四、数据库设计与迁移'});
    assert.equal(await parent.count(),1);
    const child=p.locator('#toc a').filter({hasText:'4.5 完整建表'});
    assert.ok(await child.isVisible());await parent.click();assert.equal(await child.isVisible(),false);
    await parent.click();assert.ok(await child.isVisible());await child.click();
    assert.equal(await p.evaluate(()=>document.activeElement.id),await child.getAttribute('data-toc'));
    const code=p.locator('pre code').filter({hasText:'CREATE TABLE audit_log_default'});
    assert.equal(await code.count(),1);assert.ok((await code.textContent()).includes('COMMENT ON COLUMN audit_log_default.created_at'));
    await p.screenshot({path:path.join(out,'ddl-navigation.png')});
  });
  ...
  await test('全文搜索、空结果与清空恢复', async () => {
    ...
    assert.equal(await p.locator('.section:visible').count(), 2);
  ...
  await test('Mermaid 源码编辑重绘、原始尺寸切换及导出重开', async () => {
    const q=await newPage();
    const value=await md(q);
    await q.locator('#source').fill(value.replace('flowchart TB\n','flowchart TB\n  SyntheticCheck[独立图形重绘验证]\n'));
    await q.locator('#readBtn').click();
    await q.waitForFunction(()=>Array.from(document.querySelectorAll('.diagram-view svg')).some(s=>s.textContent.includes('独立图形重绘验证')));
    const figure=q.locator('.diagram').first();
    await figure.getByRole('button',{name:'原始尺寸',exact:true}).click();
    assert.ok(await figure.getByRole('button',{name:'适应宽度',exact:true}).isVisible());
    const file=await download(q,'#exportHtmlBtn','diagram-edited.prd.html');
    const reopened=await newPage(file);
    assert.ok((await reopened.locator('.diagram-view svg').first().textContent()).includes('独立图形重绘验证'));
    await reopened.close(); await q.close();
  });
  ...
  await test('任务勾选写回 Markdown 并说明非验收', async () => {
    ...
    assert.ok((await md(p)).includes('- [x] 指定Q'));
  ...
  }, 'ZIP CRC 和两文件快照一致性由 check_delivery.py 独立验证。');
