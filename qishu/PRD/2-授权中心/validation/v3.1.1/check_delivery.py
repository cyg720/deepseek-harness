...
check('版本 0.3.0 / revision 3',state['version']=='0.3.0' and state['revision']==3)
...
check('每条用例完整正文',all('#### '+c['id']+'：'+c['title'] in body and all(c[k] for k in ['inputs','action','expected','data','role','q']) for c in cases))
...
check('30 验收记录真实标阻断',len(re.findall(r'^\| AC-\d{2} \| 阻断 \|',records,re.M))==30)
...
tables=data['TD-01']['tables'];sql=(ROOT/'DDL.md').read_text(encoding='utf-8')
...
for entity,rows in transitions.items():
    for i,r in enumerate(rows):
        check(entity+' 状态源码/表/用例 '+str(i),any(r['current']+' --> '+r['next']+': '+r['event'] in diagram for diagram in diagrams) and any('TD-08.'+entity+'['+str(i)+']' in c['inputs'] for c in cases) and any(r['current'] in line and r['next'] in line and r['event'] in line for line in body.splitlines() if line.startswith('|')))
...
check('26显式索引无重名',len(indexes)==len({x[0] for x in indexes})==26)
...
check('158字段注释且无重复定义',len(re.findall(r'COMMENT ON COLUMN (\w+\.\w+) IS',sql))==len(set(re.findall(r'COMMENT ON COLUMN (\w+\.\w+) IS',sql)))==158)
