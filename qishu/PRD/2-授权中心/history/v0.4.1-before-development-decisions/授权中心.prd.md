<!-- prd-workspace:{"schemaVersion":1,"docId":"safety-platform.authorization-center","version":"0.4.1","revision":8,"updatedAt":"2026-09-12T09:07:56.895Z"} -->
...
S-01为原始PRD，S-02为原始功能清单，S-03/S-04为原始DDL及其说明，S-05为历史模板背景，S-06为本次用户反馈。S-07为本轮13项补充反馈；S-06与S-07共同定义当前目标；旧资料冲突以反馈为准。[总体架构](../总体架构.md)的多租户描述不适用于本套单企业部署；[应用中心](../应用中心/DDL.md)负责应用业务注册和审批，两份跨模块资料未在本次修改。
...
功能判断、登录与改密、自动续期、90天清理和关联删除按已确认规则验收。用户明确本期暂不考虑并发、延迟等性能指标，AC-30标N/A；不将未压测写成容量达标。
...
| BR-03 | 应用可见范围   | 普通账号的应用可见范围由自定义权限组关联应用配置，用户直接或通过实际所属组织取得组。内置超级管理员不参与权限组，默认拥有所有权限。应用可见组不充当功能上限；同应用多可见组的具体关联模型仍待Q-03。 |
...
| BR-16 | 令牌持久化与刷新 | 仅持久化核验摘要。有效token剩余时间严格小于15天时自动向后台续期，续期成功后有效期从本次续期时间起恢复30天；剩余正好15天不触发，已到期先返回401。续期替换token还是延长原token、并发去重和Scene细节待Q-06，不继续把触发时机和寿命列为未知。 |
| BR-17 | 暂停和注销    | 冻结期间拒绝；解冻后未到期且未撤销的旧token恢复可用。主动注销、密码重置已作废的token不因解冻恢复；退出的设备范围待Q-06。                |
| BR-18 | 已删除      | N/A：委托申请、批准、拒绝、撤销和自然过期流程全部删除。                                                      |
| BR-19 | 物理删除     | 删除为物理删除；存在关联时先展示具体关联数据并提示是否一并删除，用户取消不删除，确认后按有权处理的确认范围完整执行。提交前重新核对关联及version；新增关联不得沿用旧确认悄悄删除。超级管理员不可删除，关联删除也不能删除它；SQL用RESTRICT防止无确认级联，服务事务负责已确认的清理。 |
...
| BR-28 | 时间       | 统一北京时间，接口与页面时间写明年月日时分秒；建议序列化为YYYY-MM-DD HH:mm:ss（分隔形式待Q-15）。数据库自动维护秒精度更新时间；同一秒并发仍靠version比较。 |
...
| BR-32 | 本人改密     | 当前账号可修改本人密码。有重置权限且已登录的操作人在账户中心可直接为目标账号重置随机初始密码，不需要目标账号当前登录；新随机密码满足复杂度规则，目标账号下次登录强制改密。修改/重置与该账号所有旧token失效一起完成，不影响其他账号。随机密码交付与展示期限待Q-05。 |
...
普通用户102属于部门11和21，直接关联组403，部门11关联组401，因此取得401和403。应用A可见组为401，用户可见A，也可使用403中的A操作，不要求操作同时在可见组401内。下级12的组不自动授予102。超级管理员101没有任何权限组关联，仍可见所有应用并使用全部功能；冻结/到期/改密限制仍检查。

### 2.3 管理权限与交互

账号、组织、用户组、参数、字典、权限组及关联提供各自允许的管理操作。超级管理员全权且不可删除、不分配权限组；普通管理账号按获准功能与对象执行。账号中心为有权人员提供随机重置密码入口。物理删除先显示具体关联对象、数量和影响，取消不执行，确认后重验权限、版本及关联集合；无权展示的关联不得泄密或默默删除。
...
账号组织关系使用独立关联表，没有isPrimary或主部门字段。组织树保留0根节点和org\_path；新增和移动需防环、同事务更新受影响路径。用户组只维护成员。应用授权的permission\_group\_id决定可见范围；权限组自定义配置并关联应用；当前沿用一应用一组作为数据草案，同应用是否可关联多组仍待Q-03。

操作app\_code必填、允许同应用多条操作，外键引用应用授权登记；operation\_code加入应用前缀。登记顺序是建立权限组、登记应用、上报操作、配置组内操作。operation\_ids是操作ID列表，需校验元素类型、去重和真实引用；JSONB本身不保证这些条件。外键保留RESTRICT作为防误删保护；应用层先展示引用清单并确认，再在事务中清理已确认且有权限的关联，不能自动级联。每类关联清理范围仍待Q-10。
...
以下草案中的点号操作码分隔、服务凭据绑定、一应用一可见组、RESTRICT引用保护、日志分区键及字典数组均需对应Q确认。SQL不运行旧日志清理；实现应按北京时间计算当前时刻减90天，删除早于截点的记录，任务周期待Q-09。跨表JSON引用、防环、版本比较、密码重置与所有token撤销事务由服务层补齐；脚本不假称这些逻辑已经实现。
...
在批准的独立空库执行建表后，应验证14表、默认分区、约束、版本触发器及北京时间；非法外键、重复关联、空参数、无前缀操作应拒绝。完整数据库验证和执行计划均未运行，仍须AC-29。没有旧数据，存量迁移N/A；备份与恢复操作方案待Q-16；本期不设容量性能验收。

## 五、接口操作定义

### 5.1 公共协议

统一响应使用code、message、data三字段，用户给定成功示例为 {"code":200,"message":"成功","data":{}}。code为业务结果编号；已到期token明确返回HTTP 401并让前端跳转登录。其余HTTP映射、失败code、空data类型、分页、ID和具体方法/URL仍待Q-15，不因成功体code=200就推断所有HTTP均为200。
...
| API-10 | 登录、退出、刷新令牌、注销令牌   | 当前用户或有权管理主体        | 登录账号密码；退出/注销目标登录或token标识；续期凭据传递方式待Q-06                               | 每次登录新30天token；首次强制改密；剩余不足15天自动续期至30天；到期HTTP 401前端转登录；续期token更换和退出范围Q-06 | US-04       | AC-17/AC-18/AC-19 |
...
| API-18 | 本人改密、首次改密、管理员随机重置密码 | 当前账号/受限首次登录上下文；已登录且有重置权限的人员 | 本人改密输入旧密码或批准证明、新密码；管理员重置输入目标accountId/version；新随机密码由后台产生 | 完成改密或重置、该账号旧token失效、要求重新登录；随机初始密码受控交付，展示方式Q-05 | US-15       | AC-31/AC-32/AC-39 |
...
有效token剩余不足15天触发valid→valid后台续期，成功后按续期时刻加30天；失败不假报续期成功，到期401。已改密账号被有权人员随机重置后changed→required，下次认证强制改密且所有旧token失效。两者的成功、失败和并发用例见AC-18/AC-39。

冻结是身份/应用的暂停状态，不把token改成永久撤销。token图表示自身生命周期；自动续期条件已确定，是否替换凭据仍待Q-06，不预设consumed状态。

```mermaid
stateDiagram-v2
  [*] --> valid: 签发30天token
  valid --> valid: 剩余不足15天自动续期至30天
...
| valid | 有效token剩余不足15天，后台自动续期 | valid | 成功后有效期为续期时刻加30天；正好15天不触发，到期拒绝；失败不假报成功，凭据更换方式待Q-06 |
...
委托及归档状态机N/A：功能已删除。组织、用户组、参数、字典、权限组及关联仅有CRUD和版本，不新增未批准业务状态。物理删除后对象不存在；重复删除响应由Q-15确认。日志清理是内部任务，90天截点已确认，周期和重试仍待Q-09，不新增未经确认的归档任务表。
...
| 情况                        | 结果与恢复                                      |
| ------------------------- | ------------------------------------------ |
| 无效AppKey、冻结账号、已撤销或过期token | 不返回许可；过期token返回HTTP 401并转登录，其他状态码待Q-15，不能转到委托                  |
...
| 物理删除遇引用或并发新引用             | 展示关联清单后确认；取消不删，确认时重查权限、version和关联；有变化需重新确认，不允许半删除，具体关联清理范围待Q-10     |
...
以下是当前版本的业务测试计划，不是已运行自动化测试。所有例使用独立隔离环境；超级管理员不分配组；其他管理身份按获准权限配置，不能仅靠前端角色标签认定权限。统一响应外层已确认；HTTP方法、路由和剩余业务码按Q-15确定后填写，未定项阻断对应执行。普通凭据需隔离环境真实签发，不能使用合成占位值登录。
...
| TC-31-017 | 剩余不足15天自动续期及失败恢复 | US-04 / BR-16/BR-30 | AC-18 / API-10 | TD-06/TD-07 |
...
| TC-32-004 | 关联清单确认与竞态 | US-08 / BR-19/BR-27 | AC-40 / API-07 | TD-01/TD-05/TD-06/TD-07 |
...
全部业务用例未执行；接口和数据验证需要隔离环境及批准的剩余细节。每例继承9.1的权限、清理和证据要求。
...
| 角色与前提 | 使用TD-02角色；超级管理员不配组，普通用户需对应权限；隔离环境和依赖可用。 |
...
| 预期及禁止效果 | 创建一条完整记录，返回id；不覆盖既有对象；默认值符合第三章。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 步骤 | 1. 用获准管理员准备本例临时对象及上游引用；2. 提交所列请求；3. 查详情、数据库和日志核对结果。 |
| 预期及禁止效果 | 仅目标对象按批准字段语义修改，version增加1、updatedAt由数据库维护；其他对象不变。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15/Q-12 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 步骤 | 1. 用获准管理员准备本例临时对象及上游引用；2. 提交所列请求；3. 查详情、数据库和日志核对结果。 |
| 预期及禁止效果 | 返回已建对象的获准字段，不含密码散列、token摘要等秘密。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 步骤 | 1. 用获准管理员准备本例临时对象及上游引用；2. 提交所列请求；3. 查详情、数据库和日志核对结果。 |
| 预期及禁止效果 | 能找到本例对象，筛选、稳定排序、分页和total按批准规则一致；不能看到越权对象。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 步骤 | 1. 用获准管理员准备本例临时对象及上游引用；2. 提交所列请求；3. 查详情、数据库和日志核对结果。 |
| 预期及禁止效果 | 本例无下游引用，删除后数据库记录物理消失；不能只改删除标记，其他对象不变。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15/Q-10 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 拒绝，不返回许可、不执行业务；合法AppKey对照通过后续检查 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 暂停期间拒绝；恢复后同一未到期未撤销token按用户当前权限可用 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-20/Q-08/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 前两种不签发token；合法方式按批准服务身份配置执行 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-05/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 用户取得401可见组及403操作组，允许；即使202不在401内也不应误拒绝 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-03/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 分别拒绝；不能靠AppKey或应用可见性取得全部功能 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-03/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 两个有效成员关系并存，不出现主部门；重复组合拒绝或按批准幂等返回同一关系 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-02/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 仅父子关系时无404；显式加入才取得；移除后失去，不改变其他所属部门授权 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-02/Q-08/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 成环拒绝、原树不变；合法移动全部受影响路径一致；越权目标组织拒绝 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 成员组合唯一、移除有效；仅加入用户组不增加功能权限 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 合法码归属明确；错误码拒绝，无半成品；登记不改变任何用户授权 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-18/Q-19/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 不存在/错误类型拒绝且原集合不变；重复元素按批准去重或拒绝，不能形成额外权限 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-03/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 最多一条生效；冲突不产生重复授权，失败不删除原关联 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 只维护授权信息与组关联，不新增发布审批；AppKey受控返回，明细不泄露秘密 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-03/Q-05/Q-19/Q-20/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 不谎报全部成功；已登记内容可确认，重试不意外重复创建 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-12/Q-19/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 两枚token同时有效，到期均为签发加30天；只保存摘要，不出现原文token 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-06/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 到期前可用，到期后拒绝；等于或晚于到期秒拒绝并HTTP 401、前端跳登录；错误场景不得越权，清理任务未运行不延长有效期 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-06/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |

#### TC-31-017：剩余不足15天自动续期及失败恢复

| 项目 | 内容 |
| --- | --- |
| 关联 | US-04；BR-16/BR-30；AC-18；API-10 |
| 角色与前提 | 使用TD-02角色；超级管理员不配组，普通用户需对应权限；隔离环境和依赖可用。 |
| 输入与数据 | TD-06/TD-07；renewAt15Days、renewBelow15Days、atExpiry；同入口token |
| 步骤 | 1. 剩余正好15天访问；2. 剩余14天23:59:59访问；3. 等续期成功再次检查；4. 用到期token访问；5. 模拟并发续期及响应丢失。 |
| 预期及禁止效果 | 正好15天不自动续期；不足15天自动向后台续期，成功后从续期时刻起30天，不影响其他设备；到期HTTP 401并跳登录。更换token及并发重复语义按Q-06/Q-12定稿。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-06/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 被注销token不可用且不可恢复；其他实例是否保留按Q-06已批准范围判断；重复不恢复凭证 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-06/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 冻结拒绝，解冻后未到期未撤销token恢复；已过期已注销仍拒绝 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-08/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 先显示具体引用清单并弹框确认；取消无删除；确认后只清理已授权且已展示的关联，原子物理删除。关联新增、版本变化或权限不足拒绝旧确认；超级管理员在直接和关联删除中都保留。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-10/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 中心事件自动追加、未知账号为空、凭据脱敏；业务应用执行日志不默认汇入；普通用户不能伪造或改删日志 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-09/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 始终按102权限判断；B无可见权限拒绝；不能冒用101，不能返回秘密；令牌动作复用同一实现 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 仅一个旧版本写入成功，版本为8；另一个冲突且保留输入；更新时间由数据库维护，不能因同秒而覆盖 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 按北京时间年月日时分秒传输和显示，建议YYYY-MM-DD HH:mm:ss；数据库秒精度一致，具体分隔格式待Q-15；不再要求必须带时区偏移的输入。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-11/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 权限不可核验时不允许；审计失败按Q-09处理；恢复不使用未知旧许可 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-08/Q-09/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 超长/只读/错误枚举拒绝，无越权数据或秘密；大ID按批准格式无损往返；非法排序不拼接SQL 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-01/Q-10/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 14业务表、无委托和范围列、摘要列、前缀正确；触发器和非法写入结果可核对；旧库迁移N/A，不执行生产变更 验证超级管理员标识、唯一内置身份及不可删除保护；续期后expiresAt可延长，不能仍约束为createdAt加30天。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-03/Q-05/Q-06/Q-09/Q-10/Q-14/Q-16/Q-18 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 首次仅可改密，普通业务拒绝；成功后原凭据不能使用，重新登录才能进入业务 密码少于8位或缺数字、字母、特殊字符任一类时拒绝，失败不得修改密码或撤销token。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-05/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 改密仅作用当前账号；102两枚旧token均失效，101不变；新密码登录有效，失败不得半改 密码少于8位或缺数字、字母、特殊字符任一类时拒绝，失败不得修改密码或撤销token。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-05/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 两个全码可并存；重复按批准协议去重或冲突；越权登记拒绝且不修改A操作 操作码由app_code、类名、方法名三段组成，具体分隔符待Q-18。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-18/Q-19/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 早于当前时刻减90天的日志自动物理删除；恰好90天及更新日志保留。重复清理不误删；失败可告警重试；不按3个自然月计算。 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-09 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 文字和数值有效，保存后首次消费新值；空值拒绝，原值保留；不要求重启 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-14/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 授权中心独立处理既有身份与权限，不依赖应用中心在线；初始化不得覆盖有效账号或建立通用绕过入口 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-16 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 授权中心按相同应用和操作权限判断，不返回dataScope或委托字段；业务应用可以独立拒绝某条数据，中心允许不代表执行成功 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 不合法结构拒绝且原值保留；重复项按批准规则；敏感值不因属于本中心就对所有人公开 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-14/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 无组仍全权；普通账号不能伪造isSuperAdmin；冻结/到期/首次改密限制仍拒绝 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 角色与前提 | 使用TD-02角色；超级管理员不配组，普通用户需对应权限；隔离环境和依赖可用。 |
| 输入与数据 | TD-01/TD-05/TD-06/TD-07；101，关联删除引用含101 |
| 步骤 | 分别直接删除101、编辑isSuperAdmin、在关联清理中包含101 |
| 预期及禁止效果 | 全部不得删除或改变内置身份；事务失败不半删除其他对象 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-10/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 随机初始密码至少8位且含数字字母特殊字符，102所有旧token失效，下次登录必须改密；无权请求拒绝；明文不持久化或入日志 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-05/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 步骤 | 预览引用；取消；重新预览；另建引用或改变version后确认；重新确认有效清单 |
| 预期及禁止效果 | 清单列具体数据与数量；取消不删；过时确认拒绝；最新授权清单确认后完整物理清理，隐藏关联不泄露不默删 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-10/Q-12/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 值被当作数据存储或按业务验证拒绝，不执行注入SQL；动态列名排序走白名单；普通参数仍立即生效 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-14/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
| 预期及禁止效果 | 五类业务类型可表达；未知类型和成环拒绝；类型编码/层级组合待Q-02，不自动强制只有单一路径 响应统一为code/message/data；到期HTTP 401已确认，其他映射待Q-15。 |
| 清理与重跑 | 按9.1仅清理本例对象；恢复时钟、状态与故障；不得删除内置超级管理员。 |
| 状态/依赖 | 未执行；Q-02/Q-15 |
| 实际结果/证据 | 环境、版本、执行者、时间及实际证据均未提供。 |
...
```powershell
python -X utf8 applications/PRD/授权中心/测试数据/生成测试数据.py --out-dir applications/PRD/授权中心/测试数据/v0.4.1
```
...
执行每项所列TC的全部输入、步骤和禁止副作用；当前均未执行，用户确认要求不等于实际验收通过。
...
| AC-18 | 有效token剩余严格不足15天自动续期至30天；到期HTTP401前端转登录 | TC-31-017 | TD-06/TD-07；US-04；BR-16/BR-30 |
| AC-19 | 注销不可恢复，冻结暂停、解冻恢复尚有效token | TC-31-018, TC-31-019 | TD-01/TD-08；US-01；BR-17; TD-08；US-04；BR-17 |
| AC-21 | 关联清单先确认，取消无删除，确认范围原子物理删除；保护超级管理员 | TC-31-020 | TD-01；US-08；BR-19 |
...
| AC-39 | 有权人员随机重置密码，复杂度正确、旧token失效、下次登录强制改密 | TC-32-003 | TD-01/TD-05/TD-06/TD-07；US-15；BR-31/BR-32 |
| AC-40 | 关联删除清单、取消、授权及提交前竞态重验完整 | TC-32-004 | TD-01/TD-05/TD-06/TD-07；US-08；BR-19/BR-27 |
...
| Q-03 | 部分确认 | 应用有个应用授权表；在某个应用的授权范围的人才能看得到这个应用；  那些人能操作那个应用的功能是由权限组来决定的；    | 权限组自定义配置，自定义关联应用；普通用户按取得的组获得应用可见性与功能。 | 同应用一组还是多组及共享组编辑规则未明确，保留单组数据草案。 | 权限组是自定义配置的，在自定义分配关联那些应用                                                           |
| Q-04 | 已确认  | 能不能操作这条数据由该应用的业务自行控制，暂不设定数据权限；                               | 不在授权中心设置数据权限，具体数据操作由业务应用控制。                                             | 无本期中心行权限待定项；相关字段/返回/用例已移除。                        | |
| Q-05 | 部分确认 | 普通用户就普通账号密码登录；应用只报 AppKey 就能证明身份；                            | 首次强制改密；密码至少8位且包含数字字母特殊字符；有权登录人员可随机重置他人密码，重置后旧token失效。 | 随机密码交付/展示、本人修改的原密码验证、服务凭据及密码字符定义细节待定。 | 首次登录成功 强制改初始密码；密码不得小于8位.必须包含数字字母特殊字符；重置密码需要有权限的人登录系统在账户中心 可直接重置该人的密码。重置默认密码为随机密码。 |
| Q-06 | 部分确认 | 令牌有效期30天、允许一个账号同时登录手机、电脑等设备，允许一个账户多个有效令牌；只保存核验摘要。            | 每次登录新token，30天；有效token剩余不足15天自动后台续期到30天，到期401前端转登录。 | 续期是否替换token、并发去重、摘要算法、退出范围及场景重连仍待定。 | 单个入口登录都是获取一个新的token， token过期后自动失效返回401 前端跳转登录。在token快到期小于15天就自动从后台刷新一次token保持30天。 |
...
| Q-10 | 部分确认 | 删除是彻底清除                                                      | 有关联先弹框列出具体数据并询问是否关联删除；取消不删，确认后按有权范围完整物理删除，超级管理员不可删。 | 每类关系清理范围、受限关联展示与确认请求格式仍待细化。 | 存在引用对象时提示是否进行关联删除，弹框中明确给出那些数据关联该数据                                                |
| Q-11 | 已确认 | 统一都按北京时间；到期精确到秒；更新时间由数据库自动维护                                 | 北京时间，年月日时分秒，数据库维护更新时间；到期秒拒绝，90天截点已明确。 | 序列化分隔形式由Q-15统一，例如YYYY-MM-DD HH:mm:ss。 | 年月日时分秒                                                                            |
| Q-12 | 部分确认 | 编辑接口判断version字段是否一致                                          | 编辑必须比较version是否一致。                                                      | 遗漏字段是否保持、登记和签发的幂等范围/保留期、超时查询接口仍需明确。               | |
...
| Q-15 | 部分确认 | 采用统一返回状体码和格式                                                 | 统一响应为code、message、data，成功code=200；token到期HTTP401，前端跳登录。 | 其他业务码/HTTP映射、分页、ID、URL、空data和错误详情仍待细化。 | {code:200,message:XXXX, data: Object}                                             |
...
| Q-19 | 部分确认 | 提供接口出来。应用中心自己来调用 注册过来（应用注册获取授权、把操作也上报过来）；                    | 授权中心提供接口，应用中心调用登记应用取得授权并上报操作。                                           | 上报幂等、部分失败、改名/下线/漏报及调用资格的细则仍待定。                    | |
| Q-20 | 部分确认 | 应用中心来管理注册、审批之类的。 授权中心只负责应用授权（关联权限组）；                         | 应用中心管注册审批；授权中心只管应用授权和权限组关联。                                             | 授权active/frozen的操作发起者、是否跟发布状态联动与失败补偿待定，不复制发布审批。   | |
...
本次13项补充反馈已经落实，不再追问内置超级管理员、密码复杂度、续期阈值、90天、MyBatis或统一响应外层。剩余优先收口续期token更换、随机密码交付、关联清理范围、统一错误码及操作码分隔方式。用户未填的新决策单元格保留为空，不当成否定既有决定。Q-13本期不纳入，不再阻断验收。
