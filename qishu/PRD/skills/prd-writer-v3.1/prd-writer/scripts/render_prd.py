...
The Python builder uses only the standard library; HTML embeds pinned Mermaid assets.
No business APIs, SQL or network requests are executed by the builder.
...
SCHEMA_VERSION = 1
MERMAID_DIR = Path(__file__).resolve().parents[1] / 'assets/mermaid'
MERMAID_FILE = 'mermaid-11.16.0.min.js'
...
def render_html(state: dict, template: str) -> str:
    state = validate_state(state)
    if any(template.count(token) != 1 for token in ('__CSP__', '__PRD_DATA__', '__MERMAID_JS__', '__MERMAID_LICENSE__')):
        raise ValueError('模板占位符必须各出现一次。')
    library = (MERMAID_DIR / MERMAID_FILE).read_bytes()
    if hashlib.sha256(library).hexdigest() != MERMAID_SHA256:
        raise ValueError('Mermaid 资源校验失败；请恢复包内固定版本，不使用 CDN 降级。')
    # A trusted library can contain HTML samples inside strings; escape raw-text end tags.
    javascript = re.sub(r'</script', r'<\\/script', library.decode('utf-8'), flags=re.I)
    template = template.replace('__MERMAID_JS__', javascript, 1)
    template = template.replace('__MERMAID_LICENSE__', html.escape((MERMAID_DIR / 'LICENSE').read_text(encoding='utf-8')), 1)
    hashes = []
    for script_id in ('prd-mermaid', 'prd-runtime'):
        matches = re.findall(r'<script id="' + script_id + r'">([\s\S]*?)</script>', template)
        if len(matches) != 1:
            raise ValueError('模板缺少唯一 ' + script_id + ' 脚本。')
