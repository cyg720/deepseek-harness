 ...
 * @remarks 文件说明：文件职责：实现 webhook/webhook 中 types 模块的职责，并向相邻模块提供可复用能力。；
...
export interface WebhookRule<K extends string = string> {
   ...
   * 参数说明：delivery（Readonly<VerifiedWebhookDelivery<K>>）：提供本次调用所需的数据；
   * 必须满足声明的类型及调用时序要求。；参数说明：signal（AbortSignal）：传递取消或终止信号；必须满足声明的类型及调用时序要求。；
   * 返回值：WebhookSessionRequest | null | Promise<WebhookSessionRequest | null>；
   * 调用方应按声明类型处理，不应假定未声明的附加状态。；使用示例：典型用法：在完成前置校验后调用 run(delivery, signal)，
   * 并按返回类型处理结果。
   */
  run(
