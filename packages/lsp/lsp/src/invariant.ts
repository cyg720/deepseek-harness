/**
 * ================================ 文件注释 ================================
 * 【文件职责】dsh-lsp 包的"运行时不变量伴生插件"：在全局不变量服务（ctx.invariants）中注册本包的伴生记录。当前无实际不变量断言（install 为空函数），仅作为包所有权标记。
 ...
 * 【产品维度】运行时诊断能力的组成部分：即便当前没有不变量，也预留注册位，便于将来对 lsp 服务内部状态（提供者 id、扩展名路由表）做一致性断言。
 * 【逻辑维度】定义包名常量与伴生插件名（name）和依赖注入声明（inject）→ 定义空安装函数 install → apply 将包名与安装函数注册进不变量服务并返回释放函数。
 * 【关键边界】文件骨架与其他包伴生插件雷同，被 jscpd（代码克隆检测）忽略；inject 声明必须先有 invariants 服务就绪；install 为空是刻意为之：lsp 缝的内部状态私有且原子更新，没有可独立对照的枚举快照。
 * 【新手阅读建议】先理解 ctx.invariants.register 的作用，再对照本包 index.ts 看"缝"本身；若想为 lsp 增加不变量，只需在 install 中实现断言逻辑。
 * ==========================================================================
 */
/**
 ...
 */
// 空安装函数：lsp 缝的提供者 id 与扩展名路由是私有且原子更新的状态，既无枚举快照也无生命周期事件可供独立对照，故当前不注册任何不变量。
