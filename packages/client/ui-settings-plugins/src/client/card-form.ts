/*
 * ================================ 文件注释 ================================
 * 【文件职责】每张插件卡片共享的表单模型：暂存用户输入，仅在保存时写入；
 *             字段显示有效值（用户层叠加组合层叠加模式默认），并标记用户层
 *             是否携带该字段。
 * 【技术维度】SettingsScope + SnapshotStore：写入是持久化、修订围栏的文档变更，
 *             故暂存而非边写边提交；字段有 set/clear 两种写入。
 * 【产品维度】设置页插件卡片的编辑体验：所见即所存、覆盖标记、无效拦截保存。
 * 【逻辑维度】edit/resetField 暂存 → field/shell 投影（含有效性）→ save 按计划
 *             写入并回读确认 → discard 丢弃暂存。
 * 【关键边界】有效值来自用户层覆盖而非值比较（等于默认值的覆盖仍是覆盖）；
 *             宿主是"是否接受"的唯一权威（回读判定）。
 * 【新手阅读建议】先看 CardFieldSpec 的 format/parse 对，再读 save 的回读判定。
 * ==========================================================================
 */
/**
