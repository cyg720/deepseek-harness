/**
 * ================================ 文件注释 ================================
 * 【文件职责】可配置插件页签的卡片列表：按设置命名空间分派槽位，渲染的是
 *             两张账本的交集——宿主服务的命名空间 × 注册进 settings.plugin.item
 *             的卡片。
 * 【技术维度】SnapshotStore + 共享镜像：宿主服务的集合来自 describe 镜像；
 *             卡片来自槽位账本（entries 回调）；两边的变化都会触发重发。
 * 【产品维度】设置页"插件"页签中的可配置插件卡片列表（按注册顺序、过滤未服务项）。
 * 【逻辑维度】publish 计算 served 集合 ∩ 卡片键 → 与上次快照比较（保持引用稳定）
 *             → 变化才更新。
 * 【关键边界】宿主未作答前 loaded=false（不把"未作答"说成"无插件"）；
 *             快照引用在事实未变化时保持不变（避免无关保存重渲染整张列表）。
 * 【新手阅读建议】先看 ConfigurablePluginsTabState，再读 publish 的引用稳定性逻辑。
 * ==========================================================================
 */
/**
