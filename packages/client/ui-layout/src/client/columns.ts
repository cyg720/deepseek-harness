/*
 * ================================ 文件注释 ================================
 * 【文件职责】三栏 AppFrame 的纯"让步链"列宽求解器：给定视口宽度与三栏偏好宽度，
 *             算出实际渲染宽度。
 * 【技术维度】纯函数（无副作用、无滞回）：输出只依赖（视口, 偏好）；所有几何常量
 *             都在本文件冻结（契约固定点）。
 * 【产品维度】应用主框架的三栏布局：侧边栏固定，中栏优先，详情栏最后让步并可自动收起。
 * 【逻辑维度】computeColumns 按三步让步：全放得下 → 压缩详情栏 → 自动关闭详情栏
 *             （中栏吸收剩余差额，可能低于下限）。
 * 【关键边界】侧边栏绝不让步（渲染宽度恒为拖拽偏好或收起轨道）；收起是"派生"状态，
 *             不重写偏好，窗口变宽后自动恢复。
 * 【新手阅读建议】先看各几何常量，再沿 computeColumns 的三步让步读求解逻辑。
 * ==========================================================================
 */
/**
