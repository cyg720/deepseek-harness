...
贡献项是客户端自有命令，与宿主命令同名会明确报错。它的 UI 是 popupSelect 规格或动作：裸调用消费触发 token 后运行回调，不提交消息。业务包负责自己的动作及可用性，输入框通过同一 API 注册「文件」。装饰为已有命令添加裸调用弹窗或动作，并保留其目录行、参数认领与生命周期记录；没有匹配的宿主行或可用客户端贡献时不触发。菜单查询按顺序、不区分大小写地模糊匹配命令名与标题的子序列，前缀优先，不显示小节标题。

装饰默认优先级为 0。同一既有命令允许显式不同的优先级共存，选择当前可用的最高优先级；同优先级重复注册仍报错。卸载覆盖贡献后恢复剩余呈现，不依赖安装顺序。优先级仅作用于裸命令呈现，不改变 Host 命令标识、参数或执行。

装饰也作用于可用的客户端自有贡献。原贡献必须仍然可用，装饰不能创建缺失命令或扩大能力。菜单选择与裸回车使用同一有效 UI 和附件规则，卸载装饰后恢复原贡献。
