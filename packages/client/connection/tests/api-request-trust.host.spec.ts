/*
 * 文件职责：验证 API 请求来源、Host 与可信 authority 校验规则，覆盖回环、同源、跨站和非法配置。
 * 技术维度：Node 请求头模型、Vitest 参数化测试与信任判断函数。
 * 产品维度：防止浏览器连接遭受 DNS 重绑定和跨站调用，同时允许明确配置的部署地址。
 * 逻辑维度：构造不同请求头组合，调用信任断言或布尔判断，再核对放行与拒绝结果。
 * 关键边界：测试只描述安全策略的当前行为；新增受信来源规则时必须同时补充拒绝用例。
 * 新手阅读建议：先读 request 辅助函数，再按 describe 分组比较允许与拒绝样例。
 */
...
/** 文件职责：验证 API 请求来源、Host 与可信 authority 规则。技术维度：Vitest 与请求头模型。产品维度：阻止 DNS 重绑定和跨站调用。逻辑维度：构造请求后比较放行结果。关键边界：安全规则变化须同时补拒绝用例。新手阅读建议：先比较允许和拒绝样例。 */

import { describe, expect, it } from 'vitest'
import { assertTrustedAuthority, isTrustedApiRequest } from '../src/api-request-trust.ts'

/** 中文说明：测试辅助函数 `request`；参数含义见签名，返回值供当前场景驱动或断言；例如按下方测试调用方式使用。 */
...
describe('isTrustedApiRequest', () => {
  ...
  it('accepts a declared public authority: exact on host:port entries, any port on port-less entries', () => {
    /** 中文说明：当前场景输入、传输或校验的数据；变量 `headers` 的取值由紧邻初始化或循环输入决定，仅在当前作用域使用。 */
  ...
  it('never lets stray whitespace broaden an exact-port entry to every port', () => {
    ...
    /** 中文说明：当前测试场景使用的局部状态或中间值；变量 `trusted` 的取值由紧邻初始化或循环输入决定，仅在当前作用域使用。 */
  ...
  it('refuses malformed or untrusted authorities on browser requests', () => {
    /** 中文说明：按发生顺序收集观测值的数组或记录集合；变量 `markers` 的取值由紧邻初始化或循环输入决定，仅在当前作用域使用。 */
