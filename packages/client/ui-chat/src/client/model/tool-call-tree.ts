...
export class ToolCallTree {
  private readonly childrenByParent = new Map<string, readonly ToolCallBlock[]>() // 父调用 id -> 子块列表
  private readonly depthByCall = new Map<string, number>() // 调用 id -> 深度（acceptEdge 维护）
  private readonly projectedByCall = new Map<string, ProjectedBlock>() // 调用 id -> 投影缓存
  private revision = 0 // 修订号：任何建边/重置都会递增
  private nodesCache: {
    source: readonly ConversationNode[]
    revision: number
    value: readonly ConversationNode[]
  } | null = null // 节点列表投影缓存（按源引用 + 修订号命中）
  ...
  apply(event: SessionEvent): boolean {
    if (event.type === 'tool/code-dispatch-start') {
      ...
      if (!this.acceptEdge(data.parentCallId, data.subCallId)) return true // 边不安全则消费但不建边
    ...
    const started = at === -1 ? undefined : siblings[at] // 先前 start 事件记录的运行中块（用于回填调用时刻）
  ...
  /** 递归投影单个块：子块列表优先取事件索引，否则取块自带 subCalls。 */
  ...
  private acceptEdge(parentCallId: string, subCallId: string): boolean {
    ...
    const updates = new Map<string, number>() // 本轮要落盘的深度更新
    for (const candidate of pending) {
      ...
      if (candidate.depth > MAX_TOOL_CALL_TREE_DEPTH) return false // 超深即拒绝整条边
