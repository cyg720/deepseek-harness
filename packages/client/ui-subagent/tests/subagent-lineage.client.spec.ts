/**
 * 文件职责：验证 client/ui-subagent 中 subagent lineage client spec 相关行为与失败场景。
...
describe('UI Subagent descendant projection', () => {
  ...
  it('counts nested running descendants and stops at ordinary forks', () => {
    ...
    /**
     * 常量说明：grandchild 用于处理 grandchild 相关数据，作用于当前作用域；初始化后不可重新赋值，
     * 但对象内部是否可变仍由其类型决定。
     */
    ...
    /**
     * 常量说明：forkChild 用于处理 forkChild 相关数据，作用于当前作用域；初始化后不可重新赋值，但对象内部是否可变仍由其类型决定。
     */
  ...
  it('terminates cycles and retains missing-parent aggregates', () => {
    /**
     * 常量说明：cycleA 用于处理 cycleA 相关数据，作用于当前作用域；初始化后不可重新赋值，但对象内部是否可变仍由其类型决定。
     */
    const cycleA = { id: sid('a'), parentId: sid('b'), origin: 'subagent' as const, running: false }
    /**
     * 常量说明：cycleB 用于处理 cycleB 相关数据，作用于当前作用域；初始化后不可重新赋值，但对象内部是否可变仍由其类型决定。
     */
