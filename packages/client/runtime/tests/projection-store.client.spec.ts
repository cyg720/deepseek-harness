...
declare module '@deepseek-ai/dsh-session-projection/types' {
  /** 中文说明：类型 SessionProjectionMap 约束本文件数据字段及允许取值。 */
...
describe('ProjectionValueStore semantics', () => {
  ...
  it('notifies the key face on change (batched) and not on dropped applications', async () => {
    /** 中文说明：当前状态或快照 store，取值由紧邻初始化决定，仅在当前作用域使用。 */
    const store = new ProjectionValueStore()
    /** 中文说明：测试场景的局部值 keyTicks，取值由紧邻初始化决定，仅在当前作用域使用。 */
    let keyTicks = 0
    /** 中文说明：测试场景的局部值 anyTicks，取值由紧邻初始化决定，仅在当前作用域使用。 */
    let anyTicks = 0
  ...
  it('publishes one reference-stable whole-value snapshot until a row changes', () => {
    ...
    /** 中文说明：测试场景的局部值 populated，取值由紧邻初始化决定，仅在当前作用域使用。 */
