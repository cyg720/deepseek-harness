...
export class SlotRegistry extends Service {
   ...
   */
  /**
   * 丢弃已死会话的按会话存储实例（会话服务在作用域拆除时调用；根作用域
   * 记录不受影响）。持久化状态随会话而去——从未渲染过的已死会话仍可能
   * 拥有上次页面加载留下的键，因此会瞬态物化实例只为清空存储
   * （对未持久化存储是空操作）。
   * @param sessionId 被拆除的会话。
   */
  ...
  /** 两个对象层服务挂载后构建一次；按会话的 provide 捆绑仍懒解析。 */
