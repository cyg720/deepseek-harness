/**
 * ================================ 文件注释 ================================
 * 【文件职责】会话对话（Conversation）契约的聚合定义：事件输入、匹配、
 *   位置（Location）、节点定义（Event-to-Node 状态机）、视图构建器与
 *   装配层使用的全部公共类型。
 * 【技术维度】纯类型契约 + 少量纯函数：使用声明合并的开放 Map
 *   （ConversationTurnDataMap 等）让业务包贡献具体键；跨程序统一。
 * 【产品维度】会话消息流由"定义"（Definition）驱动的状态机渲染：业务包
 *   注册定义来识别事件、累积状态、发布节点，装配器（conversation-assembler）
 *   负责调度与物化。
 * 【逻辑维度】类型按角色分组：事件/匹配（Input/Match）、位置（Step/Turn/
 *   Location）、节点（ViewNode/ChatConversationViewNode）、定义
 *   （ConversationNodeDefinition）、视图（Builder/Definition）、快照（Snapshot）。
 * 【关键边界】Location 数据与视图快照的键空间是开放（merge-extensible）的；
 *   conversationContextKey 用 kind 长度前缀保证跨 kind 不撞键。
 * 【新手阅读建议】先读 ConversationNodeDefinition（核心状态机契约），
 *   再按需看其他类型。
 * ==========================================================================
 */
