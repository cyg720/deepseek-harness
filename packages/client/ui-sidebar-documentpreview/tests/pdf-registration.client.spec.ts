...
describe('PDF registration', () => {
  it('registers a builtin complete-bytes body and removes all contributions and retained view state on dispose', async () => {
    ...
    try {
      ...
      // 官方 PDF 与 QS 使用同一 store 和注入工厂，视图切换保留页码。
      expect(entries[0]!.store).toBe(ctx.documentPdfPresentation.store)
      expect(entries[0]!.inject).toBe(ctx.documentPdfPresentation.inject)
      const face = entries[0]!.inject('s1' as SessionId, instance.actions)
      expect(entries[0]!.inject('s1' as SessionId, instance.actions)).toBe(face)
      ...
      await fiber.dispose()
      expect(ctx.get('documentPdfPresentation')).toBeUndefined()
      expect(instance.getSnapshot().byTab).toEqual({})
