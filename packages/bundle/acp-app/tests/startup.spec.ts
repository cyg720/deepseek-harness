 ...
 * @remarks 文件说明：文件职责：验证 bundle/acp-app 中 startup spec 相关行为与失败场景。；
 ...
 * 并由公开方法提供受类型约束的操作入口。；使用场景：由 bundle/acp-app 在对应插件或业务生命周期内创建和调用。 */
 ...
 * 参数说明：args（string[]）：提供本次调用所需的数据；必须满足声明的类型及调用时序要求。；返回值：{ ctx: Context;
 * exits: number[]; out: () => string; stdin: TestStdin }；调用方应按声明类型处理，
 * 不应假定未声明的附加状态。；使用示例：典型用法：在完成前置校验后调用 start(args)，并按返回类型处理结果。 */
