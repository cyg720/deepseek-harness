/**
 * ================================ 文件注释 ================================
 * 【文件职责】机器可读的 Cordis API 目录的客户端版本（由 scripts/gen-cordis-api.ts
 *             生成，勿手改）：harness 服务/事件/继承 ctx API/类型形状，供 Client
 *             侧 inspect 提供给模型（与 tool-cordis 的 api-catalog.ts 同源生成）。
 * 【技术维度】AST 遍历自动生成；查询函数 queryServiceApi/queryEventApi 提供
 *             "紧凑目录"与"精确契约"两种投影，精确契约附带被引用类型闭包。
 * 【产品维度】模型写 Client 半部前可查客户端服务/事件/槽位契约，避免依赖
 *             过时的手写文档。
 * 【逻辑维度】类型形状 → 生成数据（SERVICE_API/EVENT_API/TYPE_API/INHERITED_CTX_API）
 *             → 查询投影（queryServiceApi/queryEventApi + 类型闭包）。
 * 【关键边界】手工修改会被重新生成覆盖；数据为客户端可见子集，与 Host 侧目录
 *             内容可能不同（按平台可见性裁剪）。
 * 【新手阅读建议】理解前 80 行的接口类型 + 底部两个查询函数即可，无需通读数据。
 * ==========================================================================
 */
