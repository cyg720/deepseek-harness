/**
 * ================================ 文件注释 ================================
 * 【文件职责】浏览器槽位表面的编译期契约目录（由 scripts/gen-client-catalog.ts 生成，
 *             勿手改）：每个 SlotMap 键的可注册性、注册要求、组件收到的 props、
 *             已占用者与存在条件，经 cordis_inspect what:"client" 提供给模型。
 * 【技术维度】生成数据 + 少量导出类型；"数据只有字符串"——本模块是两平面的唯一
 *             合法会合点，因此绝不携带客户端导入。
 * 【产品维度】让模型在写 Client UI 前就知道往哪个槽位注册、要传什么、是否覆盖
 *             出厂 UI，避免注册到不存在的槽位而报错。
 * 【逻辑维度】导出类型（ClientSlotOption/ClientSlotEntry）→ CLIENT_NOTES 通用规则
 *             → CLIENT_SLOT_API 全量槽位数据。
 * 【关键边界】手工修改会被重新生成覆盖；目录是编译期契约而非单页快照——键只有在
 *             其声明者挂载时才可注册。
 * 【新手阅读建议】不需要通读数据；先读 CLIENT_NOTES 的六条通用规则，再按需查槽位。
 * ==========================================================================
 */
...
/**
 * 某种槽位基数（single/list/keyed/chain）下 register 调用要传的一个选项。
 */
...
/**
 * 一个浏览器半部可注册 UI 的槽位：键、基数、作用域、契约文案、注册选项、
 * owner/标准 props、键域、声明者与已占用者等编译期契约。
 */
