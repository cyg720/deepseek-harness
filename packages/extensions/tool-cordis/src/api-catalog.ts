/**
 * ================================ 文件注释 ================================
 * 【文件职责】机器可读的 Cordis API 目录（由 scripts/gen-cordis-api.ts 生成，勿手改）：
 *             harness 服务（摘要 + 结构化公开方法契约）、事件（模式 + 监听器契约）、
 *             继承 ctx API 与公开类型形状；cordis_inspect 把这份数据提供给模型。
 * 【技术维度】由 AST 遍历自动生成（与 docs/cordis-catalog 同源，保证不漂移）；
 *             查询函数 queryServiceApi/queryEventApi 支持"紧凑目录"与"精确契约"
 *             两种投影，精确契约附带被引用类型的传递闭包。
 * 【产品维度】让模型在写插件前拿到准确的 API 契约（签名/参数/返回/抛出），
 *             避免依赖可能过时的手写文档。
 * 【逻辑维度】类型形状定义（ApiParameter/ServiceApi*、EventApi*、InheritedApi/TypeApi）
 *             → 生成数据（SERVICE_API/EVENT_API/TYPE_API/INHERITED_CTX_API）→
 *             查询投影（queryServiceApi/queryEventApi + 类型闭包）。
 * 【关键边界】手工修改会被重新生成覆盖；数据量大（约 5000 行）属预期；
 *             referencedTypeClosure 用词边界匹配保证只收"真正被引用"的类型。
 * 【新手阅读建议】不需要通读生成数据；理解前 80 行的接口类型 + 底部两个查询函数即可。
 * ==========================================================================
 */
...
/**
 * 计算种子签名文本引用的类型闭包：反复把词边界命中 seed 的类型声明加入集合，
 * 直到不再新增（与 inspect.ts 的 typeClosure 等价，但基于全局 TYPE_API）。
 */
 ...
 */
/**
 * 投影服务目录：无 key 时为紧凑目录（每个服务 + 方法签名）；有 key 时为精确契约
 * （含可选/硬依赖两种访问方式、方法详情与被引用的类型闭包）。
 */
 ...
 */
/**
 * 投影事件目录：无 name 时为紧凑目录（每个事件一行）；有 name 时为精确契约
 * （完整描述、模式、签名、参数与被引用的类型闭包）。
 */
