/**
 * ================================ 文件注释 ================================
 * 【文件职责】pwsh 沙箱执行器内部的"运行结果分类"辅助函数：把一次失败的运行归类为
 * "运行器启动失败（spawn 失败）""运行器自身故障""策略拒绝"等可区分结局，供
 * SandboxPwshExecutor 在 run/start/onProcessDone 中判定并上报。
 * 【技术维度】纯函数集合，只依赖 node:fs 的两个同步探测（statSync/accessSync）与
 * 字符串签名匹配；通过错误对象里的 code/path/syscall 字段与 stderr 文本做分类。
 * 本文件是 bash-sandbox 版 helpers.ts 的逐调用镜像（调用方共享同一套分类方言），
 * 因此整个文件包在 jscpd:ignore 豁免区内。
