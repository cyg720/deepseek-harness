/**
 * ================================ 文件注释 ================================
 * 【文件职责】注册 dsh-bash-sandbox 包在 invariants 服务中的"包属主"声明：本包是沙箱化的
 * bash 执行器，行为契约由 ctx.shell / ctx.sandbox 缝强制执行，无独立运行时不变式，
 * 安装函数为空实现。
 * 【技术维度】Cordis 伴生插件三件套（name / inject / apply）注册到 invariants 服务；
 * 文件主体与 bash-local 等镜像文件几乎相同，故包在 jscpd:ignore 豁免区内。
