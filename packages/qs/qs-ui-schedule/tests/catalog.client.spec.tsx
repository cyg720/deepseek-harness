...
function prompts(): string[] {
  return within(screen.getByRole('list', { name: en['list.aria'] }))
    ...
    .filter(item => item.querySelector('[class*="prompt"]'))
...
describe('ScheduleCatalogAction positioning', () => {
  it('portals the catalog to the body and left-aligns it when space is available', () => {
    ...
    expect(catalog.parentElement!.style.left).toBe('240px')
    expect(catalog.parentElement!.style.top).toBe('53px')
    expect(catalog.parentElement!.style.visibility).toBe('')
  })
})
...
describe('ScheduleCatalogAction rows', () => {
  it('shows only prompt and the three derived metadata fields, with overdue records first', () => {
    ...
    // UTC 原值是本期可核对时间要求，不能继续沿用隐藏原值的旧呈现断言。
    expect(text).toContain(`UTC: ${overdue.scheduledAt}`)
    expect(rows[0]?.querySelector('time')?.getAttribute('datetime')).toBe(overdue.scheduledAt)
...
// 奇术补充：会话切换不能继承前一会话已打开的提醒目录。
it('resets the catalog on session change and exposes the read-only limitation', () => {
  const one = props([record('first', 'at', START + 1000)])
  const view = render(<ScheduleCatalogAction {...one} />)
  fireEvent.click(screen.getByRole('button'))
  expect(screen.getByText(en['list.limit'])).toBeDefined()
  view.rerender(<ScheduleCatalogAction {...one} sessionId={'other' as SessionId} />)
...
it('portals within the current themed workbench root', () => {
  const view = render(<div className="qs-root qs-dark"><ScheduleCatalogAction {...props([record('theme', 'at', START + 1000)])} /></div>)
  fireEvent.click(screen.getByRole('button'))
  const catalog = screen.getByRole('list')
  expect(catalog.parentElement?.parentElement).toBe(view.container.firstElementChild)
  expect(catalog.closest('.qs-dark')).not.toBeNull()
})
