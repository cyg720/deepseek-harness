...
Concurrent cold reads recheck that cache after their independent I/O completes. An entry published by another reader with the same persistence instance and revision is reused for preparation and projection hydration; cancellation and handle closure remain local to each read. This does not coalesce disk reads.
