 ...
 */
/**
 * 文件职责：实现 win32-abi.ts 承担的沙箱安全与权限隔离配置、协议与生命周期职责。
 ...
 */
/** 中文说明：常量 SE_GROUP_LOGON_ID 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 STANDARD_RIGHTS_WRITE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
 ...
 * rewrite DACLs.
 */
/** 中文说明：常量 GRANT_MASK 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
 ...
 */
/** 中文说明：常量 FILE_ALL_ACCESS 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 DISABLE_MAX_PRIVILEGE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const DISABLE_MAX_PRIVILEGE = 0x1
/** LUA_TOKEN: produce a limited-user (filtered admin) token. */
/** 中文说明：常量 LUA_TOKEN 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 WRITE_RESTRICTED 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：变量 WinWorldSid 保存本模块当前步骤所需的数据；取值由紧邻初始化或后续赋值决定。 */
...
/** 中文说明：变量 TokenGroups 保存本模块当前步骤所需的数据；取值由紧邻初始化或后续赋值决定。 */
...
/** 中文说明：变量 TokenDefaultDacl 保存本模块当前步骤所需的数据；取值由紧邻初始化或后续赋值决定。 */
...
/** 中文说明：常量 DACL_SECURITY_INFORMATION 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 PROCESS_QUERY_INFORMATION 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 SE_FILE_OBJECT 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 GRANT_ACCESS 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const GRANT_ACCESS = 1
/** REVOKE_ACCESS: SetEntriesInAclW removes the matching allow ACE. */
/** 中文说明：常量 REVOKE_ACCESS 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
 ...
 */
/** 中文说明：常量 SUB_CONTAINERS_AND_OBJECTS_INHERIT 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
 ...
 */
/** 中文说明：常量 STARTF_USESTDHANDLES 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 HANDLE_FLAG_INHERIT 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const HANDLE_FLAG_INHERIT = 0x1
/** INFINITE: never-timeout wait value. */
/** 中文说明：常量 INFINITE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const INFINITE = 0xFFFFFFFF
/** MAX_PATH: legacy path length bound. */
/** 中文说明：常量 MAX_PATH 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 CREATE_SUSPENDED 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 GENERIC_READ 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const GENERIC_READ = 0x80000000
/** GENERIC_WRITE: generic write access (winnt.h line ~3029). */
/** 中文说明：常量 GENERIC_WRITE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 FILE_SHARE_READ 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const FILE_SHARE_READ = 0x00000001
/** FILE_SHARE_WRITE: other opens may write (winnt.h line ~5950). */
/** 中文说明：常量 FILE_SHARE_WRITE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 LOCKFILE_EXCLUSIVE_LOCK 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
export const LOCKFILE_EXCLUSIVE_LOCK = 0x2
/** LOCKFILE_FAIL_IMMEDIATELY: fail with ERROR_LOCK_VIOLATION instead of waiting. */
/** 中文说明：常量 LOCKFILE_FAIL_IMMEDIATELY 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 ACCESS_ALLOWED_ACE_TYPE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 SID_MAX_SUB_AUTHORITIES 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 INHERITED_ACE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：常量 JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
...
/** 中文说明：变量 JobObjectExtendedLimitInformation 保存本模块当前步骤所需的数据；取值由紧邻初始化或后续赋值决定。 */
export const JobObjectExtendedLimitInformation = 9
// sizeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION), verified by abi-probe.
/** sizeof(JOBOBJECT_EXTENDED_LIMIT_INFORMATION), verified by abi-probe. */
/** 中文说明：常量 JOBOBJECT_EXTENDED_LIMIT_SIZE 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
 ...
 */
/** 中文说明：常量 JOBOBJECT_EXTENDED_LIMIT_FLAGS_OFFSET 保存本模块共享的固定值；取值依据紧邻初始化，使用时不要修改。 */
