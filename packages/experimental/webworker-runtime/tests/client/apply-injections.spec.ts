...
/**
 * 文件职责：验证 experimental/webworker-runtime 中 apply injections spec 相关行为与失败场景。
...
it('ignores script preload hints and executes script sources through the worker loader', async () => {
  /**
   * 常量说明：loadScript 用于加载 Script 相关数据，作用于当前作用域；初始化后不可重新赋值，但对象内部是否可变仍由其类型决定。
   */
  /**
   * 功能说明：处理 匿名回调 相关流程；使用场景由所在模块及调用位置决定。；返回值：由 TypeScript 根据实现推断的结果；
   * 调用方应按声明类型处理，不应假定未声明的附加状态。；典型用法：在完成前置校验后调用 匿名回调()，并按返回类型处理结果。
  ...
  /**
   * 常量说明：preload 用于处理 preload 相关数据，作用于当前作用域；初始化后不可重新赋值，但对象内部是否可变仍由其类型决定。
   */
