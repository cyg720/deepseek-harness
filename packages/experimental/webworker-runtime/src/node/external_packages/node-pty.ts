 ...
 * @remarks 文件说明：文件职责：实现 experimental/webworker-runtime 中 node pty 模块的职责，
 ...
 * @remarks 中文说明：常量说明：spawn 用于处理 spawn 相关数据，作用于当前作用域；初始化后不可重新赋值，
 * 但对象内部是否可变仍由其类型决定。 */
export const spawn = notImplementedFail(MODULE, 'spawn')

/** Open a pseudo-terminal pair (unavailable).
 * @remarks 中文说明：常量说明：open 用于打开 open 相关数据，作用于当前作用域；初始化后不可重新赋值，
 * 但对象内部是否可变仍由其类型决定。 */
