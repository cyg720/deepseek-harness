 ...
 * @remarks 文件说明：文件职责：实现 experimental/inspector 中 heap profiler 模块的职责，
 ...
 * @remarks 中文说明：功能说明：处理 heapProfilerBridgeCapability 相关流程；使用场景由所在模块及调用位置决定。
 * ；返回值：InspectorSourceCapability | undefined；调用方应按声明类型处理，不应假定未声明的附加状态。；
 * 使用示例：典型用法：在完成前置校验后调用 heapProfilerBridgeCapability()，并按返回类型处理结果。
 */
