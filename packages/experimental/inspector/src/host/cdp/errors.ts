 ...
 * @remarks 中文说明：类说明：HostCdpBridgeUnavailableError 用于集中封装 处理
 * HostCdpBridgeUnavailableError 相关状态与行为。；核心功能：通过成员字段保存状态，
 * 并由公开方法提供受类型约束的操作入口。；使用场景：由 experimental/inspector 在对应插件或业务生命周期内创建和调用。 */
export class HostCdpBridgeUnavailableError extends Error {
  /**
   * 功能说明：处理 HostCdpBridgeUnavailableError 相关流程；使用场景由所在模块及调用位置决定。
   * @param operation （string）：提供本次调用所需的数据；必须满足声明的类型及调用时序要求。
   * @returns 当前类实例；调用方应按声明类型处理，不应假定未声明的附加状态。
   * @example 通过 new HostCdpBridgeUnavailableError(operation) 创建实例，
   * 并在所属生命周期内使用。
   */
