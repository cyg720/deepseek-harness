 ...
 * @remarks 中文说明：功能说明：处理 rejectPropertyBridgeOperation 相关流程；
 * 使用场景由所在模块及调用位置决定。；返回值：never；调用方应按声明类型处理，不应假定未声明的附加状态。；
 * 使用示例：典型用法：在完成前置校验后调用 rejectPropertyBridgeOperation()，并按返回类型处理结果。
 */
