 ...
 * @remarks 文件说明：文件职责：实现 experimental/inspector 中 cordis query 模块的职责，
 ...
 * @remarks 中文说明：功能说明：执行 Inspector Query 相关流程；使用场景由所在模块及调用位置决定。；
 * 参数说明：reader（CordisRuntimeTreeReader）：提供本次调用所需的数据；必须满足声明的类型及调用时序要求。；
 * 参数说明：query（InspectorQuery）：提供本次调用所需的数据；必须满足声明的类型及调用时序要求。；
 * 返回值：Promise<InspectorQueryResult>；调用方应按声明类型处理，不应假定未声明的附加状态。；
 * 使用示例：典型用法：在完成前置校验后调用 executeInspectorQuery(reader, query)，并按返回类型处理结果。
 */
