/*
 * ================================ 文件注释 ================================
 * 【文件职责】workspace 域的 zod schema 集合（名字从地图键派生）：七个方法的
 * 请求载荷与响应值校验。
 * 【技术维度】satisfies Wire<> 对齐契约类型；WorkspaceId 品牌铸造点位于
 * sessions.schema（见其说明）并在此以域名本地名重导出；rename 用 refine 强制
 * 标题非空白。
 * 【产品维度】工作区组织界面的数据校验：列表（含归档集）、创建、重命名、删除、
 * 排序、会话移动与归档。
 * 【逻辑维度】workspaceViewSchema → 七个方法的请求/响应 schema。
 * 【关键边界】insertBefore/insertSessionBefore 的锚点省略 = 追加到末尾；
 * archiveSession 响应携带完整更新后的归档集。
 * 【新手阅读建议】与 workspace.ts 契约及 api-proxy.ts 的 workspace 域对照。
 * ==========================================================================
 */
/**
