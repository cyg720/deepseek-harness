...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  private async appendCore(id: SessionId, events: readonly SessionEvent[]): Promise<void> {
    ...
    // Agent Note).
    // 所有 append 路径（公共服务、活跃写后排空、HMR 种子/后缀收养）都汇聚于此。
    // 旧形状拒绝放在这个共享边界上，过期的 JS 插件就无法把"本后端拒读的退役形状"
    // 写进日志。未知类型守卫故意只在读取侧生效：写入时拒绝会让进行中的会话在半路
    // 失去持久性，代价高于下次加载时的明确报错（取舍见 session-log-version-mechanism
    // Agent Note）。
    assertSupportedEvents(events, id)
  ...
  private installWritePath(): void {
    ...
    // HMR：热重载不会重放 session/created，因此对已存在的活跃会话补一次初始化
    // （与 dsh-invariants 的做法一致）。
