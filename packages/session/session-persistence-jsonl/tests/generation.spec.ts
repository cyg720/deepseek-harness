...
describe('JSONL immutable generation publication', () => {
  ...
  it.each(['different', 'malformed', 'symlink', 'directory'] as const)(
    ...
    async (kind) => {
      ...
      if (kind === 'symlink') {
        // Windows junctions cover link collisions without file-symlink privileges.
        await symlink(
          process.platform === 'win32' ? root : request.sourcePath,
          request.currentPath,
      ...
      try {
        ...
        // Remove the junction before recursive fixture cleanup can reach its target.
        if (kind === 'symlink') await unlink(request.currentPath)
      }
    },
  )
