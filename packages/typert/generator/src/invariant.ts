/**
 * ================================ 文件注释 ================================
 * 【文件职责】typert-generator 包的"不变量伴随插件"（invariant companion）：按仓库约定，
 ...
 * 【产品维度】本包没有运行时不变量可校验：它是"源码项目分析器 + 构建期发射器"，运行在
 *             任何 cordis 运行时之外；它的输出契约由模型快照、可执行产物与消费方包
 *             的类型检查来保证，因此安装器是空实现。
...
// 里声明本包的存在；生成器本身不在运行时执行，没有真正的不变量需要检查。
 ...
 */
// 中文：空安装器——本包运行在任何 cordis 运行时之外，输出契约由模型快照、可执行产物
// 与消费方包的类型检查保证，因此没有任何运行时不变量需要注册，此回调什么都不做。
