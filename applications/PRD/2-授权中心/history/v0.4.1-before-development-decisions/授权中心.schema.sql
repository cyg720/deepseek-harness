-- 授权中心0.4.1新库建表草案；依据用户在Q-01—Q-23及补充表的反馈。
...
-- 候选细节：应用一组可见范围、操作码以点分隔、确认关联清理前使用RESTRICT、日志复合主键/月分区；见第十二章。
...
COMMENT ON COLUMN qs__auth__token.expires_at IS '到期时间，北京时间秒精度，签发或最近续期后30天；续期不改变创建时间';
