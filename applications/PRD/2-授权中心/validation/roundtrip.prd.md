<!-- prd-workspace:{"schemaVersion":1,"docId":"safety-platform.authorization-center","version":"0.1.1","revision":5,"updatedAt":"2026-09-12T02:51:11.557Z"} -->
# 授权中心产品需求文档
...
| 整理方式 | prd-writer v3.0（2026-09-12 读取的调整版），标准深度；核对原始资料并重新生成需求方案 |
...
编号用于追溯，不表示优先级、批准或验收状态。沿用既有两位序号；本次不重新编号，也不将 AC 改为分层编号。表中八类前缀均在本文使用。P0/P1/P2 是优先级，不是需求编号；HTML 自动生成的 RV 是评审记录编号，填写人身份未经验证。
...
| TC | 业务测试用例组 | TC-01，顺序两位编号 | 第九章；第十章验收计划 |
...
图表显示说明：配套离线编辑 HTML 使用技能提供的轻量工作台，Mermaid 为“源码展示，未图形渲染”；支持 Mermaid 的 Markdown 阅读器可渲染。同目录另附《授权中心.图示.html》，用本次正文的 Mermaid 源码生成六张静态图，断网可看；在工作台中修改图源码后，静态图示不会自动更新，需重新生成。状态表始终是可直接阅读的判定依据。
...
本章先给 Mermaid 状态图，再给对应转移表。图中的状态标识与表中“当前状态/下一状态”逐字对应，边上的事件与表中“事件”逐字对应；失败但没有完成状态转换时保持原状态。图表是目标方案，其中注明建议或 Q 的内容仍待决策。配套 HTML 为源码展示，未图形渲染。
...
|---|---|---|---|
| 资料梳理 | 原文 14 项逐项覆盖，DDL 逐表静态核对，编号追溯 | 已完成静态整理；不等同业务批准 | 输入 S-01—S-04；2026-09-12；Codex；文件哈希及结构结果见交付验证报告 |
...
> 关联编号：Q-02
> 填写人（未验证）：自动化合成验证
> 记录时间：2026-09-12T02:51:11.557Z

> 合成批注：核对组织范围。<script>window.__prdPwned=1</script>

- [ ] RV-MTXSGAX1 已处理（仅评审事项，不代表业务验收）
