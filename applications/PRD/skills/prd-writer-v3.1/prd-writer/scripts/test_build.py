...
class BuildTests(unittest.TestCase):
    ...
    def test_embedded_json_does_not_close_script(self):
        ...
        self.assertEqual(len(re.findall(r'<script(?:\s|>)', result)), 3)
    ...
    def test_csp_hash_matches_runtime(self):
        ...
        for script_id in ('prd-runtime', 'prd-mermaid'):
            runtime = re.search(r'<script id="' + script_id + r'">([\s\S]*?)</script>', result).group(1)
    ...
    def test_mermaid_assets_required_and_pinned(self):
        with tempfile.TemporaryDirectory() as temp, patch('render_prd.MERMAID_DIR', Path(temp)):
            with self.assertRaises(FileNotFoundError):
                render_html(self.state, TEMPLATE)
            (Path(temp)/'mermaid-11.16.0.min.js').write_text('window.mermaid = {}', encoding='utf-8')
            with self.assertRaisesRegex(ValueError, 'Mermaid 资源校验失败'):
                render_html(self.state, TEMPLATE)

    def test_incomplete_or_duplicated_template_rejected(self):
        for template in (TEMPLATE.replace('__MERMAID_JS__', ''), TEMPLATE + '<script id="prd-runtime">x</script>'):
            with self.assertRaises(ValueError):
                render_html(self.state, template)

    def test_offline_license_is_embedded(self):
        result = render_html(self.state, TEMPLATE)
        self.assertIn('Mermaid 11.16.0', result)
        self.assertIn('Permission is hereby granted', result)
    ...
    def test_build_pair_and_manifest(self):
        with tempfile.TemporaryDirectory() as temp:
            root=Path(temp); source=root/'input.md'; source.write_text(self.text,encoding='utf-8')
    ...
    def test_source_not_overwritten_even_with_force(self):
        with tempfile.TemporaryDirectory() as temp:
            root=Path(temp); source=root/'input.prd.md'; source.write_text(self.text,encoding='utf-8')
            with self.assertRaises(ValueError):
                build(source,root,stem='input',force=True)
            self.assertEqual(source.read_text(encoding='utf-8'),self.text)
